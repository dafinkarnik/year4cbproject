...
RTPSender::RTPSender(
    ...
    : clock_(clock),
	  ...
	  circuit_breaker_(audio ? nullptr : new CircuitBreaker(this, clock)),      
...
void RTPSender::PushRtcpIntervalToCb(uint32_t rtcp_interval, uint32_t rtcp_interval_at_receiver_estimate) {
	if (circuit_breaker_) {
		circuit_breaker_->SetRtcpInterval(rtcp_interval, rtcp_interval_at_receiver_estimate);
	}
}

int RTPSender::GetSendFrameRate() {
	return stats_proxy_->GetSendFrameRate();
}
...
int32_t RTPSender::CheckPayloadType(int8_t payload_type,
  ...
  if (payload->typeSpecific.is_video() && !audio_configured_) {
	  RtpVideoCodecTypes curr_codec = payload->typeSpecific.video_payload().videoCodecType;
      if(circuit_breaker_){
        circuit_breaker_->SetVideoCodecType(curr_codec);
      }
    video_->SetVideoCodecType(curr_codec);
    *video_type = curr_codec;
  }
  return 0;
}
...
void RTPSender::OnReceivedRtcpReportBlocks(
	...
	if (circuit_breaker_) {
		circuit_breaker_->OnReceivedRtcpReportBlocks(report_blocks);
	}        
...
bool RTPSender::TimeToSendPacket(uint32_t ssrc,
  ...
  if (circuit_breaker_) {
	  circuit_breaker_->PushPacketSize((*packet).headers_size() + (*packet).padding_size() + (*packet).payload_size());
  }
...
void RTPSender::SetSendingMediaStatus(bool enabled) {
  ...
  if(circuit_breaker_){
    circuit_breaker_->UpdateSendingStatus(enabled);
  }
}
